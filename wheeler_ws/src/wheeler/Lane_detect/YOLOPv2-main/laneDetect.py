...
# publish masked lanes rgb image with depth
...
if __name__ == '__main__':
    ...
    pub_rgd_road    = rospy.Publisher('/robocop/laneDetect_node/road', Image, queue_size=20)
    pub_depth_road  = rospy.Publisher('/robocop/laneDetect_node/depth_road', Image, queue_size=20)
    ...
    while not rospy.is_shutdown():
        ...
        ll_seg_mask_normal = (ll_seg_mask*255).astype(np.uint8)
        ll_seg_mask_3d = cv2.merge( [ll_seg_mask_normal, ll_seg_mask_normal, ll_seg_mask_normal])

        da_seg_mask = cv2.resize(da_seg_mask, (640, 360), interpolation=cv2.INTER_NEAREST)
        da_seg_mask_normal = (da_seg_mask*255).astype(np.uint8)
        da_seg_mask_3d = cv2.merge( [da_seg_mask_normal, da_seg_mask_normal, da_seg_mask_normal])


        # filter rgb frame
        filteredLine = cv2.bitwise_and( rgb_frame_cap, ll_seg_mask_3d)
        filteredLane = cv2.bitwise_and( rgb_frame_cap, da_seg_mask_3d)


        # # publish rgba masked image of road
        # alpha channel
        alpha_channel = np.full(( 360, 640, 1), 255, dtype=np.uint8)
        # convert to bgra8
        filteredLane_bgra8 = cv2.merge(( filteredLane, alpha_channel))
        # convert to img
        filteredLane_img = bridge.cv2_to_imgmsg(filteredLane_bgra8, encoding="bgra8")
        rgba_img.data = filteredLane_img.data
        pub_rgd_road.publish(rgba_img)


        # # publish masked depth data of road 
        # filter depth data
        filteredDLane = depth_frame_cap * da_seg_mask.astype(np.float32)
        filteredDLane = filteredDLane.astype(np.float32)
        # convert to img
        filteredLane_depth = bridge.cv2_to_imgmsg(filteredDLane, encoding="32FC1")
        depth_data.data = filteredLane_depth.data
        pub_depth_road.publish(depth_data)
        ...
        try:
            ...
            cv2.imshow('ll_seg_mask_normal', filteredDLane)
            cv2.imshow('da_seg_mask_normal', filteredLane)
